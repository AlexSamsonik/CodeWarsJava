...
public class DigPow {
    public static long digPow(int n, int p) {
        ...
        for (char aChar : chars) {
            result = result + Math.pow((double) Character.getNumericValue((aChar)), (double) p);
            p++;
        }
        if (result % n == 0) {
            return (new Double(result / n)).longValue();
        }
        return -1;
    }
     ...
     */
    public static long digPowCOdeWars(int n, int p) {
